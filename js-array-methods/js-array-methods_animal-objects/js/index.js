...
const firstAnimalStartingWithLetterG = animals.find((first) => {
  return first.name.startsWith("g")
})
...
const animalInEurope = animals.filter((animal) => animal.continents.includes("Europe"))
const allAnimalsInEuropeWeighLessThanOnehundred = animalInEurope.every((animal) => animal.weight < 100)
console.log(allAnimalsInEuropeWeighLessThanOnehundred)

// Hint: filter + map + reduce
const animalInAfrica = animals.filter((animal) => animal.continents.includes("Africa"))
const animalArr = animalInAfrica.map((animal) => {
  return animal.weight
})
const weightOfAllAnimalsInAfrica = animalArr.reduce((a,b) => a+b);
console.log(weightOfAllAnimalsInAfrica);


// Hint: As above, but divided by the number of animals in Africa.
const averageWeightOfAllAnimalsInAfrica = weightOfAllAnimalsInAfrica / animalInAfrica.length
console.log((averageWeightOfAllAnimalsInAfrica).toFixed(2));
